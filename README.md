...
Windowsがロックされると疑似マウス入力を自動的に停止し、ロック解除後は「ロックを防止する」がONの場合のみ自動的に再開します。
...
- ワンクリックでロック防止をON / OFF
- Windows起動時の自動起動に対応
- Windowsのロック中は疑似マウス入力を自動停止し、ロック解除後に自動再開
...
Windowsを `Win + L` などでロックすると、KeepAwakeは疑似マウス入力を自動的に停止します。

ロックを解除すると、「ロックを防止する」がONだった場合は自動的にロック防止を再開します。
手動で「ロックを防止する」をOFFにしていた場合は、ロック解除後もOFFのままです。
